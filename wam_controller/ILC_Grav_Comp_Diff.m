...
for k = 1:7
    ...
    for tk = 1:n-1
        x_1(:,tk) = x1(:,tk) + sigQ*(0.5 - rand(7,1));
         
        x1(:,tk+1) =  x2(:,tk)*Ts + x1(:,tk);
        x_1(:,tk+1) = x1(:,tk+1) + sigQ*(0.5 - rand(7,1));
     
        
            
        
        x_2(:,tk) = (x_1(:,tk+1)-x_1(:,tk))/Ts;
...
for j = 1:7
    subplot(4,2,j),plot(t_d,velKF(j,:),t_d,x_2(j,:))
    title(['measured velocities : Joint',num2str(j)])
end 
%plots
