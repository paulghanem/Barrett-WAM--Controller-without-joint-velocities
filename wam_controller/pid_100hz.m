...
for tk = 1:n1-1
    ...
    if tk == 1
        ...
        if KF == 1
            ...
            x_2(:,tk+1)= (-inv(D(pos_meas))*(transpose(G(pos_meas)))+ inv(D(pos_meas))*(U+transpose(G(pos_meas))))*Ts + Xkfm(:,1) + sigQ*randn(7,1);
            %Yp = [Xkfp(:,1)' Xkfp(:,2)']';
            % Yp = [Xkfp(:,1)' x2(:,tk+1)']';
            Yp = [x_1(:,tk+1)' Xkfp(:,1)']';
            % x2(:,tk+1) - Xkfp(:,2),pause
        elseif KF == 0
            x_2(:,tk+1)= (-inv(D(pos_meas))*(transpose(G(pos_meas)))+ inv(D(pos_meas))*(U+transpose(G(pos_meas))))*Ts + XX2 + sigQ*randn(7,1);
            Yp = [x_1(:,tk+1)' x_2(:,tk+1)']';
        end
