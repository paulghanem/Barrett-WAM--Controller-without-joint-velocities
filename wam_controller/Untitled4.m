...
U(:,1) = zeros(7,1);
ek1 = zeros(7,n1);
Ae = -(1-Ts)*eye(n);%(1-Ts)*eye(n);% - Ts*rand(n);
C= eye(14);
Ce = C;
Iq = eye(q);
Oq = zeros(q);
...
for tk = 1:n1-1
    ...
    if tk == 1
        ...
        x2(:,tk+1) = (-inv(D(x1(:,tk)))*(transpose(G(x1(:,tk)))+transpose(Co(x1(:,tk),x2(:,tk)))+bq(x2(:,tk)))+ inv(D(x1(:,tk)))*(U(:,tk)+transpose(G(x1(:,tk)))))*Ts + x2(:,tk);
