...
if Trajexp == 1
    ...
    posd = [0.5*t_d + exp(-0.5*t_d)-1.1106;
         0.3*t_d + exp(-0.3*t_d)- 2.9490;
         0.3*t_d + exp(-0.3*t_d)+1-0.192;
        -0.3*t_d - exp(-0.3*t_d)+4.1840;
        -0.3*t_d + exp(-0.3*t_d)+(-1+ 0.2825);
        -0.3*t_d + exp(-0.3*t_d)+(-1+ 0.0969);
        -0.3*t_d + exp(-0.3*t_d)+(-1 -0.2798)];
elseif Trajexp == 0
...
%                     r3 = 1.5;
                ...
                for tk = 1:n1-1
                    ...
                    if tk == 1
                        ...
                        XX2 = zeros(7,1);
                        x_2(:,tk+1)= (-inv(D(x_1(:,tk)))*(transpose(G(x_1(:,tk))))+ inv(D(x_1(:,tk)))*U)*Ts + XX2 + sigQ*(0.5 - rand(7,1));
