clear
close all
count=0;
n = 14;p = 7; q = 14;
Ts = 0.004;
Tf = 3; % Final run time
 
t_d = 0:Ts:Tf;
n1 = length(t_d);
 
veld =[0.2-0.2*(exp(-0.2*t_d));
    0.3-0.3*(exp(-0.3*t_d));
    0*ones(1,length(t_d));
    -0.3+0.3*exp(-0.3*t_d)];
posd = [0.2*t_d + exp(-0.2*t_d)-1.1106;
    0.3*t_d + exp(-0.3*t_d)- 2.9490;
    -0.192*ones(1,length(t_d));
    -0.3*t_d - exp(-0.3*t_d)+4.1840];
veld=[veld;zeros(3,n1)];
posd=[posd;0.2825*ones(1,n1);0.0969*ones(1,n1);-0.2798*ones(1,n1)];
 
yd = [posd;veld];
alfa = 1;
yd = alfa*yd;
 
t = 0:n1-1;
t = Ts*t;
%initial conditions
x1 = alfa*posd(:,1);
x2 = alfa*veld(:,1);
U = zeros(7,1);
 
Ae = 0.9*eye(n);
C= eye(14);
Ce = C;
Iq = eye(q);
Oq = zeros(q);
H = [Iq Oq Oq -Ce*Ae;Iq Oq Oq zeros(q,n);Oq Iq Oq zeros(q,n);zeros(n,q) zeros(n,q) zeros(n,q) Ae];
Ib = zeros(3*q,3*q+n);
for k11 = 1:3*q,Ib(k11,k11) = 1;end
sig = 2e-4;
sigR = sig;
sigQ = 0;%1e-4;%1e-3;
alfa = 1e0;
beta = 1e-2;
p1 = alfa*ones(1,3*q);
p2 = beta*ones(1,n);
pb = [p1 p2];
P = 1e2*diag(pb);
r1 = 5;
r2 = 50;
r3 = 1.5;
rv = 10;
rbb1 = r1*sig^2*ones(1,q/2);
rbb2 = r2*0.1*sig^2*ones(1,q/2);
rbb12 = [rbb1 rbb2];
rbb = [r3*rbb12 rbb12 rbb12];
 
Rb = 2e4*diag(rbb);
Q = 1e-6*eye(n); % We may need to set Q to zero
X = [x1(:,1)' x2(:,1)']';
Y = C*X;
Yp = Y;
x_1(:,1) = x1(:,1);
% Kalman filter parameters
Akf = [1 Ts;0 1];
% Kkf = [3.9685e-01  2.4559e+01]';
Kkf = [ 2.7888e-02
   9.8596e-02];
Xm = zeros(2,7);
for tk = 1:n1-1
    Uk(:,tk) = U;
    Be = Ts*[zeros(7,7);inv(D(x_1(:,tk)))];
    Delyd = [(yd(:,tk+1)-yd(:,tk))];
    Del2yd = Delyd*Delyd';
   
    Gd = [Del2yd Oq Oq zeros(q,n); Oq Oq Oq zeros(q,n); Oq Oq Oq zeros(q,n);zeros(n,q) zeros(n,q) zeros(n,q) Q];
   
    
    
    EE = [Ce*Be;zeros(q,p);zeros(q,p);-Be];
    K = inv(EE'*EE)*EE'*H*P*Ib'*inv(Ib*P*Ib' + Rb);
    PHI = H - EE*K*Ib;
    %if max(abs(eig(PHI))) > 1, disp('max(abs(eig(PHI))) > 1'),tk, pause(1),end
   
    P = PHI*P*PHI' + EE*K*Rb*K'*EE' + Gd;
    K1 = K(:,1:q);
    K2 = K(:,q+1:2*q);
    K3 = K(:,2*q+1:3*q);
    normK1(tk) = norm(K1);
    normK2(tk) = norm(K2);
    normK3(tk) = norm(K3);
   
   
    if tk == 1
        E = yd(:,tk) - Y ;
        Ep = yd(:,tk+1)-Yp ;
        U = U + K1*Ep + K2*E;
        Ym = Y;
        Y = Yp;
       
        x_1(:,tk) = x1(:,tk) + sig*(0.5 - rand(7,1));
        % Modeling the robot
        x1(:,tk+1) =  x2(:,tk)*Ts + x1(:,tk);
        x2(:,tk+1) = (-inv(D(x1(:,tk)))*(transpose(G(x1(:,tk))))+ inv(D(x1(:,tk)))*U)*Ts + x2(:,tk);
        %
        x_1(:,tk+1) = x_1(:,tk); %Since at t=0, velocity is zero
        XX2 = zeros(7,1);
        x_2(:,tk+1)= (-inv(D(x_1(:,tk)))*(transpose(G(x_1(:,tk))))+ inv(D(x_1(:,tk)))*U)*Ts + XX2 + sigQ*randn(7,1);
        Yp = [x_1(:,tk+1)' x_2(:,tk+1)']';
        
    else
        Em = yd(:,tk-1) - Ym ;
        E = yd(:,tk) - Y ;
        Ep = yd(:,tk+1)-Yp ;
        U = U + K1*Ep + K2*E + K3*Em;
        Ym = Y;
        Y = Yp;
        % Modeling the robot
        x1(:,tk+1) =  x2(:,tk)*Ts + x1(:,tk);
        x2(:,tk+1) = (-inv(D(x1(:,tk)))*(transpose(G(x1(:,tk))))+ inv(D(x1(:,tk)))*U)*Ts + x2(:,tk);
        %
        x_1(:,tk) = x1(:,tk) + sig*(0.5 - rand(7,1)); % In experimental work you do not add noise instead you only use measurement of position
       
        % Kalman filter
       
        vm = (1/Ts)*(x_1(:,tk) - x_1(:,tk-1)); % Estimate of speed at tk-1
        for kk = 1:7
            yk(:,kk) = vm(kk) - [1 0]*Xm(:,kk);
            Xkf(:,kk) = Xm(:,kk) + Kkf*yk(:,kk); % Corrected state at tk
            Xm(:,kk) = Akf*Xkf(:,kk); % predicted state at tk+1
            vv(kk) = Xm(1,kk);
            vplus(kk) = [1 Ts]*Xm(:,kk); % Estimated v at tk+1: This is what we need
        end
       
        
            x_1(:,tk+1) =  vv'*Ts + x_1(:,tk); % Here we are estimating position using the extimate of x2 and measurement of x1 at present time
            x_2(:,tk+1) =   vplus';
       
        Yp = [x_1(:,tk+1)' x_2(:,tk+1)']';
    end
    YYY = [x1(:,tk);x2(:,tk)];
    Ek(tk) = max(abs(yd(:,tk) - YYY));
    normP(tk) = norm(P);
   
end
figure
plot(normK1),hold,plot(normK2,'k'),plot(normK3,'g')
legend('||K_1||','||K_2||','||K_3||'),xlabel('sec'),grid
figure
subplot(2,1,1),plot(t(1:length(Ek)),Ek),ylabel('max(|E|)')
subplot(2,1,2),plot(t(1:length(normP)),normP),ylabel('||P||')
xlabel('sec')
figure
for j = 1:7
    subplot(4,2,j),plot(t_d,x_2(j,1:n1),t_d,x2(j,1:n1))
    title(['Velocity: Joint',num2str(j)])
end
figure
for j = 1:7
    subplot(4,2,j),plot(t_d(1:length(Uk)),Uk(j,:))
    title(['Torque: Joint',num2str(j)])
end
figure
for j = 1:7
    subplot(4,2,j),plot(t_d,yd(j,:),t_d,x1(j,1:n1))
    title(['Position: Joint',num2str(j)])
end
figure
for j = 1:7
    subplot(4,2,j),plot(t_d,yd(j+7,:),t_d,x2(j,1:n1))
    title(['Velocity: Joint',num2str(j)])
end
figure
for j = 1:7
    subplot(4,2,j),plot(t_d,abs( yd(j,:)-x1(j,1:n1) ) )
    title(['Position Error: Joint',num2str(j)])
end
 
